...
const CheckRegistration = () => {
  ...
  const fetchAvailablePrograms = async (client: any) => {
    try {
      ...
      if (category?.includes('job card') || category?.includes('others')) {
        ...
        const {
          data: categories
        } = await supabase.from('employment_categories').select('id').eq('name', 'foodelife');
        ...
        const {
          data: categories
        } = await supabase.from('employment_categories').select('id').eq('name', 'entrelife');
        if (categories && categories.length > 0) {
          programsQuery = programsQuery.eq('category_id', categories[0].id);
        }
        ...
        const {
          data: categories
        } = await supabase.from('employment_categories').select('id').eq('name', 'organelife');
        if (categories && categories.length > 0) {
          programsQuery = programsQuery.eq('category_id', categories[0].id);
        }
        ...
        const {
          data: categories
        } = await supabase.from('employment_categories').select('id').eq('name', 'farmelife');
        if (categories && categories.length > 0) {
          programsQuery = programsQuery.eq('category_id', categories[0].id);
        }
      }
  ...
  const fetchUserNotifications = async (client: any, registrations: any[]) => {
    try {
      ...
      const {
        data: notifications,
        error
      } = await (supabase as any).from('notifications').select('*').in('target_id', categoryIds).eq('type', 'category').eq('is_active', true).order('created_at', {
        ascending: false
      });
      if (error) {
      ...
      const {
        data: programNotifications,
        error: progError
      } = await (supabase as any).from('notifications').select('*').eq('type', 'program').eq('is_active', true).order('created_at', {
        ascending: false
      });
      if (progError) {
  ...
  const checkRegistrations = async () => {
    ...
    try {
      ...
      const {
        data: client,
        error: clientError
      } = await supabase.from("registered_clients").select("*").eq("mobile_number", mobileNumber).single();
      ...
      const {
        data: regs,
        error: regsError
      } = await supabase.from("employment_registrations").select(`
          *,
          employment_categories (
            name,
            description
      ...
      const registrationsWithPrograms = await Promise.all((regs || []).map(async reg => {
        if (reg.category_id) {
          const {
            data: programs
      ...
      if ((client.category?.toLowerCase().includes('job card') || client.category?.toLowerCase().includes('others')) && regs && regs.length > 0) {
        toast({
  ...
  const applyForProgram = async (programId: string, programName: string) => {
    ...
    if (registrations.length > 0) {
      ...
      const activeRegistrations = registrations.filter(reg => reg.status === 'pending' || reg.status === 'approved');
  ...
  const requestStopProgram = async (registrationId: string, categoryName: string) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('employment_registrations').update({
  ...
  const handleDialogApplication = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('employment_registrations').insert({
  ...
  return <div className="container mx-auto p-4 sm:p-6 max-w-4xl">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl sm:text-2xl">Check Registration Status</CardTitle>
          <CardDescription className="text-sm sm:text-base">താഴെ നിങ്ങളുടെ മൊബൈൽ രജിസ്ട്രേഷൻ ചെയ്ത മൊബൈൽ നമ്പര് നല്‍കി ചെക്ക് ചെയ്യുക</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            <div className="flex-1">
              <Label htmlFor="mobile" className="text-sm sm:text-base">Mobile Number</Label>
              <Input id="mobile" type="tel" placeholder="Enter your mobile number" value={mobileNumber} onChange={e => setMobileNumber(e.target.value)} className="text-sm sm:text-base" />
            </div>
            <div className="flex flex-col sm:flex-row sm:items-end gap-2">
              <Button onClick={checkRegistrations} disabled={loading} className="text-sm sm:text-base">
                {loading ? "Checking..." : "Check Status"}
              </Button>
              {(clientData || registrations.length > 0) && <Button onClick={reset} variant="outline" className="text-sm sm:text-base">
                  Reset
                </Button>}
            </div>
          </div>

          {clientData && <Tabs defaultValue="details" className="w-full">
                  ...
                  <CardHeader>
                    ...
                    <CardDescription>
                      <div className="space-y-2">
                        ...
                        <div>
                          {clientData.category?.toLowerCase().includes('job card') || clientData.category?.toLowerCase().includes('others') ? <span className="text-green-600">✓ Can apply to all programs (Special qualification)</span> : <span className="text-amber-600">⚠ Limited to category-specific programs</span>}
                        </div>
                      </div>
                    </CardDescription>
                  </CardHeader>
                    ...
                    <Card className="mt-4">
                      ...
                      <CardContent>
                        <div className="space-y-3">
                          {userNotifications.length === 0 ? <div className="text-center py-6 text-muted-foreground">
                              ...
                              {userNotifications.map((notification, index) => <div key={notification.id} className="p-4 border rounded-lg bg-blue-50 border-blue-200">
              ...
              <TabsContent value="registrations" className="space-y-4">
                {registrations.length > 0 ? <div className="space-y-4">
                    {registrations.map(reg => <Card key={reg.id}>
                        <CardContent className="pt-6">
                          ...
                          {reg.available_programs && reg.available_programs.length > 0 && <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                              <div className="flex justify-between items-center mb-2">
                                <h4 className="font-medium text-sm">Available Programs for this Category:</h4>
                                <Button size="sm" variant="destructive" onClick={() => requestStopProgram(reg.id, reg.employment_categories?.name || '')}>
                                  Request Stop/Multi-Program
                                </Button>
                              </div>
                              <div className="space-y-2">
                                {reg.available_programs.map((program, index) => <div key={index} className="border border-blue-200 p-2 rounded bg-white">
                                    ...
                                    {program.conditions && <p className="text-xs text-orange-600 mt-1">Conditions: {program.conditions}</p>}
                                    {program.sub_projects && <p className="text-xs text-purple-600 mt-1">Sub-project: {program.sub_projects.name}</p>}
                                  </div>)}
                              </div>
                            </div>}
              ...
              <TabsContent value="programs" className="space-y-4">
                {availablePrograms.length > 0 ? <div className="space-y-4">
                    <Card>
                      <CardHeader>
                      ...
                      <CardContent>
                        <div className="rounded-md border overflow-x-auto">
                          <Table>
                            ...
                            <TableBody>
                              {availablePrograms.map(program => <TableRow key={program.id}>
                                  ...
                                  <TableCell>
                                    {program.sub_projects ? <Badge variant="outline">{program.sub_projects.name}</Badge> : "-"}
                                  </TableCell>
                                  ...
                                  <TableCell>
                                    <Button size="sm" onClick={() => applyForProgram(program.category_id, program.name)}>
                                      Apply
                                    </Button>
                                  </TableCell>
                                </TableRow>)}
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4 py-4">
                <div>
                  ...
                  <Textarea id="experience" placeholder="Describe your relevant work experience..." value={experience} onChange={e => setExperience(e.target.value)} className="mt-1" rows={3} />
                </div>
                <div>
                  <Label htmlFor="skills" className="text-sm font-medium">
                    Skills *
                  </Label>
                  <Textarea id="skills" placeholder="List your relevant skills and capabilities..." value={skills} onChange={e => setSkills(e.target.value)} className="mt-1" rows={3} />
                </div>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <div className="space-y-2">
                  ...
                  <Input id="blockingTitle" value={blockingTitle} onChange={e => setBlockingTitle(e.target.value)} className="text-lg font-medium" placeholder="Enter dialog title" />
                </div>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div>
                  ...
                  <Textarea id="blockingMessage" value={blockingMessage} onChange={e => setBlockingMessage(e.target.value)} className="min-h-[120px] mt-2" placeholder="Enter blocking message content" />
                </div>
              </div>
